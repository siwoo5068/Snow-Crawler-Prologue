...
### 6. FogManager.cs 고도화 (동적 다크 블리자드 시스템)
- 게임을 진행할수록 시야가 극한으로 좁아지는 **동적 눈보라(안개) 악화 시스템**을 추가했습니다.
- **주요 기능:**
  1. 별장(SafeZone) 안에서는 항상 아늑하고 밝은 옅은 안개가 낍니다.
  2. **야외로 출격할 때마다 안개가 점점 더 짙어지고, 색상이 검푸른색(어둠)으로 변합니다.** (기본 세팅: 5번째 출격 시 최대치 도달)
  3. 초반에는 손전등 없이도 실루엣이 보이지만, 후반부 출격에서는 칠흑 같은 눈보라가 쳐서 **손전등이 없으면 한 치 앞도 볼 수 없는 공포 생존 경험**이 만들어집니다.
  4. 공간(별장/야외)을 넘어갈 때 안개 농도와 색상이 부드럽게 스르륵 변환됩니다.
- **집에서 씬에 세팅하는 법:**
  1. 기존 `FogManager`가 이미 부착되어 있다면 추가 작업 필요 없음! (자동으로 `SortieManager`를 찾아 출격 횟수를 동기화합니다)
  2. Inspector에서 `Max Fog Sortie Count`나 색상을 취향껏 조절해 보세요.

...
### 5. FlashlightController.cs 고도화 (배터리 및 공포 깜빡임 시스템)
- 손전등에 **배터리 수명**과 **공포 연출**을 추가했습니다.
- **주요 기능:**
  1. 손전등을 켜두면 배터리가 서서히 닳음. (기본 약 1분 지속)
  2. 배터리가 20% 이하로 떨어지면 불빛이 무작위로 깜빡거림 (호러 분위기 극대화) + 자막 출력("배터리가 얼마 남지 않았어...").
  3. 배터리가 완전히 0이 되면 강제로 꺼지며 더 이상 켤 수 없음 + 자막 출력("손전등이 꺼졌다...").
  4. **별장(SafeZone)으로 무사히 귀환**할 때마다 배터리가 자동으로 100% 꽉 충전됨! (SortieManager와 연동 완료)
- **집에서 씬에 세팅하는 법:**
  1. 이미 플레이어에게 붙어있는 컴포넌트이므로, Inspector에서 배터리 소모 속도나 깜빡임 기준치만 취향껏 조절하시면 됩니다. (추가 세팅 없음!)

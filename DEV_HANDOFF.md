...
> "최신 상태"란: **집 PC에 있는 최신 씬 파일 + git pull로 받은 최신 스크립트가 합쳐진 완전한 프로젝트 상태**를 의미합니다.
> 이 세이브포인트가 있어야, 이후 작업에서 문제가 생겼을 때 "씬도 스크립트도 전부 최신인 깨끗한 상태"로 되돌릴 수 있습니다.

```
사용자에게 확인: "git pull은 이미 하셨나요?"
→ 아직이면: git pull 먼저 실행
→ 했으면: Unity 열기 → Ctrl+S → git add . → git commit -m "안전 세이브포인트" → git push
→ 커밋 해시를 사용자에게 알려주기
→ 그 다음에 이 문서를 계속 읽기
```

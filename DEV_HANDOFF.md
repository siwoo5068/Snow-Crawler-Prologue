...
## 🚨 AI 필독: 이 문서를 숙지하기 전에 반드시 아래를 먼저 수행하세요!

**DEV_HANDOFF.md 숙지 전, 현재 최신 상태 저장부터 합니다.**

세이브포인트를 **2개** 만들어야 합니다. 순서를 반드시 지키세요!

### 세이브포인트 ①: 집 PC 원본 상태 백업 (git pull 전!)
> git pull을 하기 전, 집 PC의 현재 상태를 그대로 저장합니다.
> 만약 학교에서 만든 스크립트 자체에 문제가 있으면 이 시점으로 롤백합니다.
```
1. Unity 열기 → Ctrl+S (씬 저장)
2. git add .
3. git commit -m "세이브포인트1: git pull 전 집PC 원본 상태"
4. git push
5. 이 커밋 해시를 사용자에게 알려주기
```

### 세이브포인트 ②: 집 원본 + 학교 스크립트 통합 상태 (git pull 후!)
> 학교 스크립트를 받은 뒤, 씬과 합쳐진 깨끗한 통합 상태를 저장합니다.
> 이후 인수인계 작업(오브젝트 배치 등) 중 문제가 생기면 이 시점으로 롤백합니다.
```
1. git pull (학교 스크립트가 내려옴)
2. Unity에서 씬이 정상인지 확인 → Ctrl+S
3. git add .
4. git commit -m "세이브포인트2: 최신씬 + 최신스크립트 통합 완료"
5. git push
6. 이 커밋 해시도 사용자에게 알려주기
```

> 🛡️ **이중 안전망 요약:**
> - 인수인계 작업 중 문제 → 세이브포인트 ②로 롤백 (스크립트는 살리고 씬만 되돌림)
> - 학교 스크립트 자체가 문제 → 세이브포인트 ①로 롤백 (학교 작업 전으로 완전 복원)
>
> **두 세이브포인트가 완료된 후에 이 문서의 나머지 내용을 숙지하세요.**
...
### 0단계: 세이브포인트 확인
> 위의 🚨 섹션에서 세이브포인트 ①, ②를 이미 만들었다면 이 단계는 완료입니다.
> 만들지 않았다면 반드시 위로 돌아가서 먼저 수행하세요!
...
> 안녕! 이 프로젝트의 `DEV_HANDOFF.md` 파일을 열어줘. 맨 위에 적힌 지시사항(최신 상태 저장)부터 따라한 다음에 나머지 내용을 숙지해 줘.
>
